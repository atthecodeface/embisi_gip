...
/*a Constants
 */
constant integer io_simple_fifo_address_size=8; // Use 8 for status, 9 for data
...
typedef enum [3]
{
    simple_fifo_op_reset, // zeros ptrs, empties FIFO
    simple_fifo_op_write_cfg, // write base address (8/9 bits, added to ptrs), size (8/9 bits), watermark (8/9 bits); empties FIFO, zeros ptrs also
    simple_fifo_op_inc_write_ptr, // If full, does nothing, excepts sets overflow
    simple_fifo_op_inc_read_ptr, // If empty, does nothing, excepts sets underflow
} t_simple_fifo_op;

/*a Modules
 */
extern module io_simple_fifo( clock int_clock "main system clck",
