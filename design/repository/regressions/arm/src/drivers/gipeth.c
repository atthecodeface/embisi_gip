...
// set header_details[0] to point to us
    ...
    GIP_SET_SEMAPHORES(SEM_ETH_TX_DATA_VWM); // watermark is low to start with, so set our semaphore
    GIP_POST_TXD_0( GIP_POSTBUS_COMMAND( 0, 0, 0 ) ); // header details: route=0
    GIP_POST_TXC_0_IO_CMD( 0, 0, 0, 0x11 );
    NOP;NOP;NOP;NOP;NOP;NOP;
    GIP_POST_TXD_0( (IO_A_SLOT_ETHERNET_0<<0) | (1<<2) | (1<<3) | (1<<4) | (0<<5) | (0<<postbus_command_source_io_cmd_op_start) | (0<<postbus_command_source_io_length_start) | (SEM_NUM_ETH_STATUS_VNE<<postbus_command_target_gip_rx_semaphore_start) ); // ingress=1, cmd_status=1, fifo=0, empty_not_watermark=1, level=0, length=0, cmd_op=0 (read data length 0)
    GIP_POST_TXC_0_IO_CMD( 0, 0, 0, 0x14 );
    NOP;NOP;NOP;NOP;NOP;NOP;
    GIP_POST_TXD_0( (IO_A_SLOT_ETHERNET_0<<0) | (0<<2) | (0<<3) | (0<<4) | (0<<5) | (0<<postbus_command_source_io_cmd_op_start) | (0<<postbus_command_source_io_length_start) | (SEM_NUM_ETH_TX_DATA_VWM<<postbus_command_target_gip_rx_semaphore_start) ); // ingress=0, cmd_status=0, fifo=0, empty_not_watermark=0, level=0, length=0, cmd_op=0 (read data length 0)
    ...
    GIP_SET_THREAD(ETH_THREAD_NUM,gipeth_hw_thread,(((SEM_ETH_STATUS_VNE|SEM_ETH_SOFT_IRQ)>>(ETH_THREAD_NUM*4))<<4)|1); // set thread 6 startup to be ARM, on semaphore 24, 25 or 26 set, and the entry point
    MK_INT_EN();
}
...
// 	stats->tx_bytes+=skb->len;
    analyzer_enable();
        ...
        GIP_SET_SEMAPHORES( SEM_ETH_SOFT_IRQ );        // hit hardware thread with softirq
    }
    else
    {
#ifdef REGRESSION
