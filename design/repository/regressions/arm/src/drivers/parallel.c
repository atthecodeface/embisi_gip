...
extern void parallel_frame_capture_init( int slot, int line_skip, int nlines, int init_gap, int pixel_gap, int pixels_per_line )
{
    ...
    fc_data[4] = line_skip;
    uart_tx_string_nl("Config capture init");
    uart_tx_hex8( nlines );
    uart_tx_string(" : ");
    uart_tx_hex8( pixels_per_line );
    uart_tx_string(" : ");
    uart_tx_hex8( init_gap );
    uart_tx_string(" : ");
    uart_tx_hex8( pixel_gap );
    uart_tx_string(" : ");
    uart_tx_hex8( line_skip );
    uart_tx_string_nl(" : ");
    parallel_config_fsm( slot, sizeof(fc_states)/sizeof(fc_states[0]), fc_states, fc_conds );

    GIP_CLEAR_SEMAPHORES( 1<<31 );
 }
