...
static void timer_1_entry( void )
{
...

    {
        int i;
        unsigned int t, s;
        __asm__ volatile (" .word 0xec00ce08 ; mov r0, r12 ; bl uart_tx_hex8 " : : : "r0" );
...
//        GIP_TIMER_WRITE( 1, t );
        ...
        __asm__ volatile (" mov r0, %0, lsl #8 \n orr r0, r0, #0x96 \n orr r0, r0, #0x16000000 \n .word 0xec00c40e \n mov r0, r0" :  : "r" ('*') : "r0" ); // extrdrm (c): cddm rd is (3.5 type 2 reg 0) (m is top 3.1 - type of 7 for no override)
    }
...
//    uart_tx_string_nl("C");
    ...
    __asm__ volatile( " mov r0, lr ; bl uart_tx_hex8 " : : : "r0" );
    __asm__ volatile( " mov r0, sp ; bl uart_tx_hex8 " : : : "r0" );
    __asm__ volatile( " mov r0, pc ; bl uart_tx_hex8 " : : : "r0" );
    return 0;
}
