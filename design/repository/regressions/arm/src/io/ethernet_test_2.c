...
//    ethernet_tx_buffer( buffer );
    if (rxed_byte_length!=expected_length)
    {
    ...
    for (i=4; i<expected_length-8; i++)
    {
        int j;
        j = (i&~3) | (3-(i&3));
        if (buffer->data[j]!=expected_data)
        {
...
extern int test_entry_point()
{
    ...
    ethernet_init( POSTBUS_ROUTE_GIP, POSTBUS_ROUTE_IO_A_SLOT, IO_A_SLOT_ETHERNET_0, 1, 0 );
