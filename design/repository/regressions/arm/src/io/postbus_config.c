...
extern void postbus_config( int rx_0, int tx_0, int rx_1, int tx_1 )
{
    ...
    GIP_POSTBUS_IO_FIFO_CFG( 0, 0, 2, 64/2,   15, 3 );  // par a cmd
    GIP_POSTBUS_IO_FIFO_CFG( 0, 0, 3, 96/2,   15, 3 );  // par b cmd
    GIP_POSTBUS_IO_FIFO_CFG( 1, 0, 0,  192, 1855, 3 );  // eth tx data
    GIP_POSTBUS_IO_FIFO_CFG( 1, 0, 1,  128,   31, 3 );  // ss txd
    GIP_POSTBUS_IO_FIFO_CFG( 1, 0, 2,    0,   31, 3 );  // par a txd, not used yet
    GIP_POSTBUS_IO_FIFO_CFG( 1, 0, 3,  160,   31, 3 );  // par b txd
    ...
    GIP_POSTBUS_IO_FIFO_CFG( 0, 1, 2, 64/2,   15, 3 ); // par a status
    GIP_POSTBUS_IO_FIFO_CFG( 0, 1, 3, 96/2,   15, 3 ); // par b status
    GIP_POSTBUS_IO_FIFO_CFG( 1, 1, 0,  192, 1023, 3 ); // erx data
    GIP_POSTBUS_IO_FIFO_CFG( 1, 1, 1,  128,   31, 3 ); // ss rx data
    GIP_POSTBUS_IO_FIFO_CFG( 1, 1, 2, 1280,  127, 3 ); // par rxdata
    GIP_POSTBUS_IO_FIFO_CFG( 1, 1, 3,  160,   31, 3 ); // par b rxdata
